"""text-to-speech — OpenAI-compatible TTS using Chatterbox-Turbo.

OpenAI-compatible API surface:

    POST /v1/audio/speech   — request schema (input/voice/response_format/speed)
                              `X-Visemes` response header (base64 Rhubarb JSON)
                              `Access-Control-Expose-Headers` CORS hint
    POST /v1/audio/speech/stream — NDJSON, sentence-by-sentence (see streaming.py)
    GET  /v1/voices         — language/default/native/aliases catalog
    GET  /health            — {"status":"healthy"}

Engine notes:
  - Chatterbox-Turbo (Resemble AI, MIT). Zero-shot — voice selection works
    by routing the request's `voice` name to a reference WAV in
    /app/voices/ (uploaded) or /opt/chatterbox-voices/ (bundled).
...
# Logging — prefer the shared Loki logger when running inside the compose
# stack, fall back to stdlib in SOLO mode.
if not config.SOLO:
...
# report an honest content-type rather than mislabeling the bytes.
_SF_ENCODERS = {
...
app = FastAPI(title="HavenCore TTS (Chatterbox-Turbo)")


class SpeechRequest(BaseModel):
    # Field names mirror OpenAI's audio.speech schema exactly.
    input: str
...
def list_voices() -> JSONResponse:
    ...
    return JSONResponse({
        # The `language` slot carries the engine name — Chatterbox is not
        # language-keyed the way some TTS engines are, but clients that
        # surface this field in a UI still get something meaningful.
        "language": "chatterbox-turbo",
        "default": voices.default_voice(reg),
        "native": native,
        # Fields the agent uses to drive the voice delete/manage UI:
        "user": user,
