...
| Device Actions | 4 — `set_alarm` on the user's phone (intent-fire); `take_photo` / `identify_object_in_photo` / `read_text_from_image` round-trip a JPEG through `/api/companion/upload`, with the latter two chaining server-side to the vision pipeline before returning to the LLM | [tools/device-action.md](tools/device-action.md) |
