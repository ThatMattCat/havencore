...
- `selene_agent/orchestrator.py` — event-based agent loop (THINKING / TOOL_CALL / TOOL_RESULT / REASONING / METRIC / SUMMARY_RESET / DEVICE_ACTION / AVATAR_STATE / DONE / ERROR). `DEVICE_ACTION` is a companion-app side-channel emitted whenever the executed tool is in the `DEVICE_ACTION_TOOLS` allowlist (`set_alarm`, `take_photo`, `identify_object_in_photo`, `read_text_from_image`, `who_is_in_view`). Tools in `PRE_EXECUTE_DEVICE_ACTION_TOOLS` (the camera tools) ship the frame *before* the body runs because the handler awaits an upload; `set_alarm` ships after `tool_result`. Tools in `COMPANION_UPLOAD_TOOLS` route through `_handle_companion_camera()` instead of `mcp_manager.execute_tool` — the upload future + blob store live in the agent process and are unreachable from the mcp-tools server process. Tools in `VISION_CHAINED_TOOLS` additionally call `api/vision._call_vision()` in-process after the upload arrives, returning `{status: "captured_and_analyzed", identification|text}` to the LLM. Tools in `FACE_IDENTIFY_TOOLS` (just `who_is_in_view`) instead POST the captured JPEG bytes (read from the in-process BlobStore — no self-HTTP) to the face-recognition service's `/api/identify` and return `{status: "captured_and_recognized", found, name?, confidence?, ...}`. The companion app fires the matching platform intent (set_alarm) or launches the camera capture flow (camera tools); older app builds drop the frame silently — see [tools/device-action.md](tools/device-action.md). `REASONING` is chain-of-thought surfaced from reasoning-capable models (e.g. Qwen3.8 via vLLM's `--reasoning-parser qwen3`); the WS-frame form is dashboard-only (yielded on `/ws/chat`, filtered out of `/api/chat`'s `events[]`, naturally absent from `/v1/chat/completions`). The same CoT is also normalized onto the assistant message as `reasoning_content` and appended to `self.messages` — the chat model's template reads that field and renders `<think>…</think>` only for assistant messages newer than the most recent user message (i.e. the in-progress agentic tool-call loop, where the model expects to see its own prior reasoning before the next call). For completed prior turns the template auto-emits empty `<think></think>`, so retaining the field across turns is harmless. `AVATAR_STATE` is another companion-app side-channel: when the final assistant message carries an `<<EXPRESSION:...>>` sentinel, the loop fires one `AVATAR_STATE` event just before `DONE` (forwarded on `/ws/chat` as `{"type":"avatar_state","expression":"..."}`) to drive the Live2D avatar's facial expression; the per-turn marker is kept in conversation history so the model keeps emitting it, and is stripped only from the `DONE`/TTS content. with per-turn LLM and tool-call timing instrumentation. Each conversation session owns its own `AgentOrchestrator` instance (messages, `session_id`, `last_query_time`); singletons (OpenAI client, MCP manager, model, tools) are shared across all sessions. `SUMMARY_RESET` is yielded at turn start when either the idle-timeout check (`_check_session_timeout`) or the context-size check (`_check_context_size`, which summarizes-and-resets when estimated message tokens exceed the active provider's context budget) compacted the session just before the incoming turn — the background idle sweep likewise resets on either axis (idle OR size). The client renders an inline "Conversation summarized" marker above the next assistant reply.
