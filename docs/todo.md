...
- **Calendar mutation tools (add / edit / delete events).** The HA calendar surface today is read-only — `ha_get_calendar_events` (`mcp_homeassistant_tools/mcp_server.py:675`) lists upcoming events, but there's no way for the LLM to create or modify them. Reminders cover one-shot pings, but a real calendar entry is what the user actually wants for "schedule a dentist appointment for next Tuesday at 10am" or "move my 3pm to 4pm". HA exposes `calendar.create_event`, `calendar.update_event`, and `calendar.remove_event` services; ship matching tools (`ha_create_calendar_event`, `ha_update_calendar_event`, `ha_delete_calendar_event`) alongside the existing read tool, with explicit datetime + summary + description params so a smaller model doesn't have to guess the service-call shape. Check which calendar entities support write (Local Calendar does; some integrations are read-only) before settling on the param surface.
